...
# Partitionnement des lignes

rows = [y for y in range(height) if y % nbp == rank]
local_height = len(rows)
...
deb = time()
for y_local, y_global in enumerate(rows):
    for x in range(width):
        c = complex(-2. + scaleX*x, -1.125 + scaleY * y_global)
        local_convergence[y_local, x] = mandelbrot_set.convergence(c, smooth=True)
fin = time()
print(f"Partition {rank}: {local_height} lignes calculées en {fin-deb}")

# Rassembler l'image
data = None
if rank == 0:
    data = np.empty((nbp, local_height, width),dtype=np.double)
comm.Gather(local_convergence,data,root=0)

if rank == 0:
    full_convergence = np.empty((height, width),dtype=np.double)
    for r in range(nbp):
        full_convergence[r::nbp, :] = data[r, :, :]
